...
export class DockerServer extends BaseMCPServer {
    ...
    async registerTools() {
        // Container lifecycle
        this.registerTool('docker_create_container', 'Create a new Docker container', ContainerCreateSchema, createToolHandler(async (params) => {
            return this.createContainer(params);
        }));
        this.registerTool('docker_start_container', 'Start a Docker container', ContainerActionSchema, createToolHandler(async ({ containerId }) => {
            return this.startContainer(containerId);
        }));
        this.registerTool('docker_stop_container', 'Stop a Docker container', z.object({
            containerId: z.string().describe('Container ID or name'),
        ...
        this.registerTool('docker_remove_container', 'Remove a Docker container', z.object({
            containerId: z.string().describe('Container ID or name'),
        ...
        this.registerTool('docker_restart_container', 'Restart a Docker container', z.object({
            containerId: z.string().describe('Container ID or name'),
            timeout: z.number().default(10).describe('Timeout in seconds'),
        }), createToolHandler(async ({ containerId, timeout }) => {
        ...
        this.registerTool('docker_list_containers', 'List Docker containers', z.object({
        ...
        this.registerTool('docker_inspect_container', 'Get detailed container information', ContainerActionSchema, createToolHandler(async ({ containerId }) => {
            return this.inspectContainer(containerId);
        }));
        this.registerTool('docker_container_logs', 'Get container logs', ContainerLogsSchema, createToolHandler(async (params) => {
            return this.getContainerLogs(params);
        }));
        this.registerTool('docker_container_stats', 'Get container resource usage statistics', z.object({
            containerId: z.string().describe('Container ID or name'),
        ...
        // Container execution
        this.registerTool('docker_exec', 'Execute command in running container', ContainerExecSchema, createToolHandler(async (params) => {
            return this.execInContainer(params);
        }));
        // Image management
        this.registerTool('docker_list_images', 'List Docker images', z.object({
            all: z.boolean().default(false).describe('Show all images (including intermediates)'),
            filters: z.record(z.string()).default({}).describe('Filters to apply'),
        }), createToolHandler(async ({ all, filters }) => {
            return this.listImages(all, filters);
        }));
        this.registerTool('docker_pull_image', 'Pull Docker image from registry', ImagePullSchema, createToolHandler(async (params) => {
            return this.pullImage(params);
        }));
        this.registerTool('docker_build_image', 'Build Docker image', ImageBuildSchema, createToolHandler(async (params) => {
            return this.buildImage(params);
        }));
        this.registerTool('docker_remove_image', 'Remove Docker image', z.object({
        ...
        // Volume management
        this.registerTool('docker_create_volume', 'Create Docker volume', VolumeCreateSchema, createToolHandler(async (params) => {
            return this.createVolume(params);
        }));
        this.registerTool('docker_list_volumes', 'List Docker volumes', z.object({
            filters: z.record(z.string()).default({}).describe('Filters to apply'),
        }), createToolHandler(async ({ filters }) => {
            return this.listVolumes(filters);
        }));
        this.registerTool('docker_remove_volume', 'Remove Docker volume', z.object({
        ...
        // Network management
        this.registerTool('docker_create_network', 'Create Docker network', NetworkCreateSchema, createToolHandler(async (params) => {
            return this.createNetwork(params);
        }));
        this.registerTool('docker_list_networks', 'List Docker networks', z.object({
            filters: z.record(z.string()).default({}).describe('Filters to apply'),
        }), createToolHandler(async ({ filters }) => {
            return this.listNetworks(filters);
        }));
        this.registerTool('docker_remove_network', 'Remove Docker network', z.object({
            networkId: z.string().describe('Network ID or name'),
        }), createToolHandler(async ({ networkId }) => {
            return this.removeNetwork(networkId);
        }));
        // System operations
        this.registerTool('docker_system_info', 'Get Docker system information', z.object({}), createToolHandler(async () => {
            return this.getSystemInfo();
        }));
        this.registerTool('docker_system_prune', 'Clean up unused Docker resources', z.object({
    ...
    async createContainer(params) {
        ...
        const memoryLimit = this.parseResourceLimit(params.memory, 'memory');
    ...
    async stopContainer(containerId, timeout) {
        try {
            const container = this.docker.getContainer(containerId);
    ...
    async removeContainer(containerId, force, volumes) {
        try {
            const container = this.docker.getContainer(containerId);
    ...
    async restartContainer(containerId, timeout) {
        try {
            const container = this.docker.getContainer(containerId);
    ...
    async listContainers(all, filters) {
        try {
    ...
    async getContainerStats(containerId, stream) {
        try {
            const container = this.docker.getContainer(containerId);
    ...
    async removeImage(imageId, force, noPrune) {
        try {
    ...
    async removeVolume(volumeName, force) {
        try {
    ...
    async systemPrune(volumes, all, filters) {
        try {
    ...
    async trackOperation(operation, target, metadata = {}) {
        try {
    ...
    async handleCustomMethod(method, params) {
        switch (method) {
            ...
            case 'toggleOperationTracking':
                return this.toggleOperationTracking(params);
            case 'updateResourceLimits':
                return this.updateResourceLimits(params);
            default:
    ...
    async toggleOperationTracking(params = {}) {
    ...
    async updateResourceLimits(params) {
        if (params.memory) {
