...
export class FilesystemServer extends BaseMCPServer {
    ...
    async registerTools() {
        // File operations
        this.registerTool('fs_read_file', 'Read contents of a file', ReadFileSchema, createToolHandler(async (params) => {
            return this.readFile(params);
        }));
        this.registerTool('fs_write_file', 'Write content to a file', WriteFileSchema, createToolHandler(async (params) => {
            return this.writeFile(params);
        }));
        this.registerTool('fs_delete_file', 'Delete a file', FilePathSchema, createToolHandler(async ({ path: filePath }) => {
            return this.deleteFile(filePath);
        }));
        this.registerTool('fs_copy_file', 'Copy a file to another location', CopyFileSchema, createToolHandler(async (params) => {
            return this.copyFile(params);
        }));
        this.registerTool('fs_move_file', 'Move/rename a file', MoveFileSchema, createToolHandler(async (params) => {
            return this.moveFile(params);
        }));
        // Directory operations
        this.registerTool('fs_list_directory', 'List contents of a directory', ListDirectorySchema, createToolHandler(async (params) => {
            return this.listDirectory(params);
        }));
        this.registerTool('fs_create_directory', 'Create a directory', CreateDirectorySchema, createToolHandler(async (params) => {
            return this.createDirectory(params);
        }));
        this.registerTool('fs_delete_directory', 'Delete a directory', z.object({
        ...
        // File information
        this.registerTool('fs_get_stats', 'Get file or directory statistics', FilePathSchema, createToolHandler(async ({ path: filePath }) => {
            return this.getStats(filePath);
        }));
        this.registerTool('fs_exists', 'Check if a file or directory exists', FilePathSchema, createToolHandler(async ({ path: filePath }) => {
        ...
        // File watching
        this.registerTool('fs_watch_path', 'Watch a path for file system changes', WatchPathSchema, createToolHandler(async (params) => {
            return this.watchPath(params);
        }));
        this.registerTool('fs_unwatch_path', 'Stop watching a path', z.object({
        ...
        this.registerTool('fs_list_watchers', 'List active file watchers', z.object({}), createToolHandler(async () => {
            return this.listWatchers();
        }));
        // Search
        this.registerTool('fs_search_files', 'Search for files by name or content', SearchFilesSchema, createToolHandler(async (params) => {
            return this.searchFiles(params);
        }));
        // Utility operations
        this.registerTool('fs_get_working_directory', 'Get current working directory', z.object({}), createToolHandler(async () => {
            return {
                cwd: process.cwd(),
    ...
    async readFile(params) {
        ...
        try {
            ...
            const options = { encoding: params.encoding };
            if (params.offset !== undefined || params.length !== undefined) {
                ...
                try {
                    ...
                    const content = params.encoding === 'utf8'
                        ...
                        : buffer.toString(params.encoding, 0, result.bytesRead);
                    return {
                        path: params.path,
                        content,
    ...
    async writeFile(params) {
        ...
        // Check content size
        const contentSize = Buffer.byteLength(params.content, params.encoding);
    ...
    async listDirectory(params) {
        ...
        try {
            ...
            if (params.pattern) {
                ...
                filteredResults = results.filter(item => minimatch.minimatch(item.name, params.pattern));
            }
            return {
                path: params.path,
    ...
    async createDirectory(params) {
        const absolutePath = this.resolvePath(params.path);
        this.validatePath(absolutePath);
        const options = { recursive: params.recursive };
        if (params.mode !== undefined) {
            options.mode = params.mode;
        }
